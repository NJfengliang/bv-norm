...
def main(args):  
    ...
    y_train = torch.Tensor(Data['velocity_z'][0:ntrain:,:] )
    y_test  = torch.Tensor(Data['velocity_z'][-ntest:,:] )
    ...
    for i in range(5):
        ...
        for args in [
                        { 'modes'   : 100,
                          ...
                          'batch_size': 10, 
                          'epochs'    : 2000,
                          'data_dir'  : '../data/BloodFlow.mat',
                          'num_train' : 350,
                          'num_test'  : 150,
                          'CaseName'  : 'blood_z_w4_2000_07/' + str(i), #此处保留测试记录用
                          'basis'     : 'LBO',
                          'lr'        : 0.001},
                    ]:
