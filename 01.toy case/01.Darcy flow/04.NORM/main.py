...
def main(args):  
    ...
    ntrain = args.num_train
    ntest  = args.num_test
    x_train = torch.Tensor(Data['f_bc'][0:ntrain])
    x_test  = torch.Tensor(Data['f_bc'][-ntest:])
    
    y_train = torch.Tensor(Data['u_field'][0:ntrain])
    y_test  = torch.Tensor(Data['u_field'][-ntest:])
    ...
    eigenvalue, E = DLO.GetE(L_matrix) #
    E_inverse = E.T
    ...
    for i in range(5):
        ...
        for args in [
                        { 'modes'   : 50,
                          ...
                          'batch_size': 10, 
                          'epochs'    : 2000,
                          'data_dir'  : '../Data/mult_holes.mat',
                          'num_train' : 1400,
                          'num_test'  : 600,
                          'CaseName'  : 'darcy_m_F_2000_0711/' + str(i), #L1D:1D LBO;F:FFT
                          'basis'     : 'LBO',
                          'lr'        : 0.001},
                    ]:
